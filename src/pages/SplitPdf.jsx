...
export default function SplitPDF() {
  ...
  const uploadFile = async () => {
    if (!file) return;
    if (!pageRange.trim()) {
      alert("Please enter a page range before splitting.");
      return;
    }
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `${file.name.replace(".pdf", "")}_split.pdf`);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4">
      ...
      <p className="text-gray-600 text-center mb-8 max-w-lg">
        Upload your PDF, enter page ranges (e.g., <span className="font-mono">2-3, 7, 10-100, 109</span>),
        and download a new PDF instantly. Maximum file size 50MB.
      </p>
      ...
      {file && (
        <div className="mt-6 w-full max-w-md">
          <input
            type="text"
            placeholder="Enter page range (e.g., 1-3, 5, 10-12)"
            value={pageRange}
      ...
      {file && (
        ...
        >
          {loading ? "Splitting..." : "Split & Download PDF"}
        </button>
      )}
